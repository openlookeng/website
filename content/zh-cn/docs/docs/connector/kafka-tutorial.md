...
本教程假定用户熟悉openLooKeng且本地安装有可用的openLooKeng（见[手动部署openLooKeng](../installation/deployment.html)）。教程将专注于设置Apache Kafka并将其与openLooKeng集成。
...
启动[openLooKeng CLI](../installation/cli.html)：
...
Kafka中的数据可以使用openLooKeng查询，但并不是呈实际表的形式。原始数据可以通过`_message`和`_key`列获得，但不会解码为列。由于样本数据是JSON格式，因此可以使用openLooKeng内置的[json](../functions/json.html)对数据进行切片。
