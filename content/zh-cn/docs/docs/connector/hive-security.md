...
可以通过在Hive目录属性文件中设置`hive.security`属性来启用[Hive](./hive.html)的授权检查。此属性必须是下列值之一：
...
| `file`| 授权检查使用Hive配置属性`security.config-file`指定的配置文件来执行。有关详细信息，请参阅[基于文件的授权](./hive-security.html#基于文件的授权)。|
| `sql-standard`| 按照SQL标准，只要用户具有所需的权限，就可以执行这些操作。在这种模式下，openLooKeng基于Hive元存储中定义的权限对查询执行授权检查。若要更改这些权限，使用[GRANT](../sql/grant.html)和[REVOKE](../sql/revoke.html)命令。有关详细信息，请参阅[基于SQL标准的授权](./hive-security.html#基于sql标准的授权)。|
...
Hive连接器提供了额外的安全选项来支持配置为使用[Kerberos](./hive-security.html#Kerberos支持)的Hadoop集群。

访问`HDFS (Hadoop Distributed File System)`时，openLooKeng可以[模拟](./hive-security.html#终端用户模拟)运行查询的最终用户。这可以与HDFS权限和`ACLs (Access Control Lists)`一起使用，为数据提供进一步的安全性。
...
有关设置Kerberos身份验证的信息，请参阅[协调节点Kerberos认证](../security/server.html)和[CLI接口的Kerberos认证](../security/cli.html)。
...
[Keytab文件附加信息](./hive-security.html#keytab文件附加信息)
