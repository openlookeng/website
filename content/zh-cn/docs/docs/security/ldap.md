...
| `http-server.authentication.password.user-mapping.file`    | 包含用户映射规则的JSON文件。详见 [认证用户映射](./user-mapping.html)。 |
...
使用[Java密钥库文件验证](tls.html#java密钥库文件验证)中的方法验证密钥库文件的密码并查看其内容。
