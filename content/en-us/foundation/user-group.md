+++
title = "User Group"
+++


# User Group 

### Mission



<ul class="ug-list">
    <li class="ug-item">
        <span class="avatar"></span>
        <ul class="desc">
            <li class="name">Serven Green</li>
            <li>CCO Community Lead</li>
            <li><span></span>Core Focus: xxx,xxx</li>
            <li><span></span>Senior Software Engieer @ XXX Company</li>
            <li><span></span>xxxxxxxx@email.com</li>
            <li><span></span>pensonal_homepage.com</li>
        </ul>
    </li>
    <li class="ug-item">
        <span class="avatar"></span>
        <ul class="desc">
            <li class="name">Serven Green</li>
            <li>CCO Community Lead</li>
            <li><span></span>Core Focus: xxx,xxx</li>
            <li><span></span>Senior Software Engieer @ XXX Company</li>
            <li><span></span>xxxxxxxx@email.com</li>
            <li><span></span>pensonal_homepage.com</li>
        </ul>
    </li>
    <li class="ug-item">
        <span class="avatar"></span>
        <ul class="desc">
            <li class="name">Serven Green</li>
            <li>CCO Community Lead</li>
            <li><span></span>Core Focus: xxx,xxx</li>
            <li><span></span>Senior Software Engieer @ XXX Company</li>
            <li><span></span>xxxxxxxx@email.com</li>
            <li><span></span>pensonal_homepage.com</li>
        </ul>
    </li>
    <li class="ug-item">
        <span class="avatar"></span>
        <ul class="desc">
            <li class="name">Serven Green</li>
            <li>CCO Community Lead</li>
            <li><span></span>Core Focus: xxx,xxx</li>
            <li><span></span>Senior Software Engieer @ XXX Company</li>
            <li><span></span>xxxxxxxx@email.com</li>
            <li><span></span>pensonal_homepage.com</li>
        </ul>
    </li><li class="ug-item">
        <span class="avatar"></span>
        <ul class="desc">
            <li class="name">Serven Green</li>
            <li>CCO Community Lead</li>
            <li><span></span>Core Focus: xxx,xxx</li>
            <li><span></span>Senior Software Engieer @ XXX Company</li>
            <li><span></span>xxxxxxxx@email.com</li>
            <li><span></span>pensonal_homepage.com</li>
        </ul>
    </li>
    
</ul>